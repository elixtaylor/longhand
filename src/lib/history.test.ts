...
describe('share links', () => {
  ...
  it('survives characters that need escaping', () => {
    const state = {
      input: 'what is 50% of 200 & more?',
      solverId: 'percentages',
    };
    const back = decodeShare(encodeShare(state));
  ...
  it('rejects malformed or excessively long shared state', () => {
    expect(decodeShare('#q=x&t=not%20an%20id')).toBeNull();
    expect(decodeShare(`#q=${'x'.repeat(2_001)}`)).toBeNull();
  });
});
...
describe('history', () => {
  ...
  it('drops corrupt stored entries rather than restoring unsafe state', () => {
    localStorage.setItem(
      'longhand.history',
      JSON.stringify([
        { input: 'x', solverId: '../bad', methodId: 'm', at: 1 },
      ]),
    );
    expect(loadHistory()).toEqual([]);
  });
