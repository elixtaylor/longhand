...
const answers = (raw: string) =>
  runWorked(raw).parts.map((p) =>
    p.result.ok ? p.result.solution.answerLatex : `FAILED`,
  );

describe('questions that span two topics', () => {
  it('splits on an explicit "then"', () => {
    expect(ids('x^2 - 4 = 0 then d/dx x^2 - 4')).toEqual([
      'quadratics',
      'differentiate',
    ]);
  });
...
describe('a later part referring to an earlier one', () => {
  it('uses the earlier answer for "the answer"', () => {
    ...
    expect(w.parts[0].result.ok && w.parts[0].result.solution.answerLatex).toBe(
      'x = -9',
    );
    // 20% of −9 = −1.8
    expect(
  ...
  it('uses the earlier expression for "differentiate it"', () => {
    ...
    expect(w.parts[1].result.ok && w.parts[1].result.solution.answerLatex).toBe(
      "f'(x) = 2x + 5",
    );
  });
  ...
  it('picks the named one out of several roots', () => {
    ...
    const w = runWorked(
      'solve 2x^2 + 7x - 4 = 0 then find 15% of the larger root',
    );
    expect(
      w.parts[1].result.ok && w.parts[1].result.solution.answerLatex,
    ).toContain('0.0');
...
describe('reading an answer back as a value', () => {
  ...
  it('refuses an answer that is not a single value', () => {
    ...
    expect(
      plainAnswer('\\bar{x} = 9.2, \\quad \\text{median} = 10'),
    ).toBeNull();
