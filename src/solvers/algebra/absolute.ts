...
function extractAbs(
  ...
  if (fn) {
    ...
    if (depth === 0)
      return {
        inner: text.slice(start, i - 1),
        before: text.slice(0, fn.index),
        after: text.slice(i),
      };
  }
  ...
  return {
    inner: text.slice(first + 1, second),
    before: text.slice(0, first),
    after: text.slice(second + 1),
  };
}
...
function parse(raw: string): Parsed | null {
  ...
  if (!lhsAbs || lhsAbs.before.trim() !== '' || lhsAbs.after.trim() !== '')
    return null;
  ...
  if (rhsAbs && rhsAbs.before.trim() === '' && rhsAbs.after.trim() === '') {
    try {
      ...
      return {
        ...
        insideLatex,
        rhs: { kind: 'abs', text: rhsAbs.inner, latex: rhsLatex },
      };
    } catch (e) {
  ...
  return {
    insideText: lhsAbs.inner,
    insideLatex,
    rhs: { kind: 'expr', text: rhsRaw, latex: toLatex(rhsExpr) },
  };
}
...
function caseSteps(
  ...
  text: string,
  methodId: string,
): { steps: Step[]; answer?: string } | null {
  const r = solveCase(text, methodId);
  if (!r.ok) return null;
  const steps: Step[] = [
    { note: label, latex: r.solution.headline.replace(/^Solve \$|\$$/g, '') },
    ...r.solution.steps.slice(1),
  ];
...
function solveImpl(input: string, methodId: string): SolveResult {
  ...
  if (!parsed) {
    return {
      ok: false,
      error:
        'Write an absolute-value equation, e.g.  |x - 3| = 5  or  |2x + 1| = |x - 4|.',
    };
  }
  ...
  const steps: Step[] = [
    {
      ...
      latex: headline.replace(/^Solve \$|\$$/g, ''),
    },
  ];
  ...
  if (rhs.kind === 'const' && rhs.value < 0) {
    ...
    return {
      ok: true,
      solution: { headline, methodName: 'Case split', steps },
    };
  }
  ...
  if (!case1 || !case2) {
    return {
      ok: false,
      error: `Couldn't solve one of the two cases from $${insideLatex}$.`,
    };
  }
  ...
  if (rhs.kind === 'expr') {
    const values = [
      ...numericSolutionsOf(posText),
      ...numericSolutionsOf(negText),
    ];
    const rejected = values.filter(
      (x) => evaluateExpr(parseExpr(rhs.text), { x }) < -1e-9,
    );
    if (rejected.length) {
  ...
  return {
    ...
    solution: {
      ...
      methodName: 'Case split',
      steps,
      answerLatex: answers.length ? answers.join(', \\quad ') : undefined,
    },
  };
}
...
export const absoluteSolver: Solver = {
  ...
  blurb:
    'Solve |expression| = k by splitting into the two cases it could have come from.',
  ...
  methods: [
    {
      ...
      name: 'Case split',
      blurb:
        'Drop the bars into the two cases they could hide, then solve each.',
    },
  ],
