...
export const probabilitySolver: Solver = {
  ...
  solve(input, methodId): SolveResult {
    ...
    if (pa && pb) {
      ...
      const steps: Step[] = [
        {
          note: 'For independent events, multiply the probabilities.',
    ...
    return {
      ok: false,
      error:
        'Try  3 out of 8,  or give two events like  P(A)=0.4, P(B)=0.3 union.',
    };
  },
};
...
function readNamed(input: string, name: string): Rational | null {
  const re = new RegExp(
    `P\\s*\\(\\s*${name}\\s*\\)\\s*=\\s*([0-9./%]+(?:\\s*out\\s+of\\s*\\d+)?)`,
    'i',
  );
  const m = input.match(re);
  if (m) return readProbability(m[1]);
  const short = input.match(
    new RegExp(`\\bp${name.replace(/[^a-z]/gi, '')}\\s*=\\s*([0-9./%]+)`, 'i'),
  );
