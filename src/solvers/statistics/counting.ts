...
function parse(input: string, methodId: string): Query {
  ...
  if (
    /choose|select|combination|committee|team/.test(lower) &&
    nums.length >= 2
  ) {
    return {
      kind: 'combination',
  ...
  if (nums.length >= 2) {
    return {
      kind: methodId === 'permutation' ? 'permutation' : 'combination',
      n: Math.max(nums[0], nums[1]),
      r: Math.min(nums[0], nums[1]),
    };
  }
...
export const countingSolver: Solver = {
  ...
  detect(input) {
    ...
    if (/\bchoose\b|\bcombination|\bpermutation|how many ways/i.test(input))
      return 0.9;
    return 0;
  },
  solve(input, methodId): SolveResult {
    ...
    if (n < 0 || r < 0)
      return {
        ok: false,
        error: 'Counting problems need non-negative whole numbers.',
      };
    if (n > MAX_N)
      return {
        ok: false,
        error: `That number is too large to work out exactly (keep n under ${MAX_N}).`,
      };
    if (kind !== 'factorial' && r > n) {
      return {
        ok: false,
        error: `You can’t choose ${r} things from only ${n}.`,
      };
    }
    ...
    if (kind === 'factorial') {
      ...
      const steps: Step[] = [
        {
          note: 'A factorial counts the ways to arrange every item.',
    ...
    if (kind === 'permutation') {
      ...
      const steps: Step[] = [
        {
          note: 'Order matters here, so use a permutation.',
          latex: `^{n}P_{r} = \\dfrac{n!}{(n-r)!}`,
        },
        {
          note: 'Substitute the numbers.',
          latex: `^{${n}}P_{${r}} = \\dfrac{${n}!}{(${n}-${r})!} = \\dfrac{${n}!}{${n - r}!}`,
        },
        {
          note: 'Most of the factorial cancels, leaving a short product.',
          latex: `= ${terms.join(' \\times ')}`,
        },
        {
          note: 'Multiply.',
          latex: `^{${n}}P_{${r}} = ${value}`,
    ...
    const steps: Step[] = [
      {
        note: 'Order does not matter here, so use a combination.',
        latex: `^{n}C_{r} = \\dbinom{n}{r} = \\dfrac{n!}{r!\\,(n-r)!}`,
      },
      {
        note: 'Substitute the numbers.',
