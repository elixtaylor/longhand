...
function parse(input: string): Parsed {
  ...
  let m = s.match(
    new RegExp(
      `${N}\\s*%\\s*(increase|rise|more).*?\\b(?:is|to|=)\\s*${N}`,
      'i',
    ),
  );
  ...
  m = s.match(
    new RegExp(
      `${N}\\s*%\\s*(decrease|discount|off|less|reduction).*?\\b(?:is|to|=)\\s*${N}`,
      'i',
    ),
  );
  ...
  // "increase 80 by 15%"
  m = s.match(
    new RegExp(`(?:increase|raise|add).*?${N}\\s*by\\s*${N}\\s*%`, 'i'),
  );
  if (m) return { kind: 'increase', a: Number(m[1]), b: Number(m[2]) };
  m = s.match(
    new RegExp(
      `(?:decrease|reduce|discount|subtract).*?${N}\\s*by\\s*${N}\\s*%`,
      'i',
    ),
  );
  ...
  // "80 increased by 15%"
  m = s.match(
    new RegExp(`${N}\\s*(?:increased|raised)\\s*by\\s*${N}\\s*%`, 'i'),
  );
  if (m) return { kind: 'increase', a: Number(m[1]), b: Number(m[2]) };
  m = s.match(
    new RegExp(`${N}\\s*(?:decreased|reduced)\\s*by\\s*${N}\\s*%`, 'i'),
  );
  if (m) return { kind: 'decrease', a: Number(m[1]), b: Number(m[2]) };
  ...
  throw new Error(
    'Try  20% of 150,  increase 80 by 15%,  or  30 as a percentage of 150.',
  );
}
...
export const percentageSolver: Solver = {
  ...
  methods: [
    {
      id: 'auto',
      name: 'What it needs',
      blurb: 'Reads the wording and picks the right percentage calculation.',
    },
    {
      id: 'decimal',
      name: 'Decimal multiplier',
      blurb:
        'Turn the percentage into a decimal and multiply — the quickest route.',
    },
    {
      id: 'unitary',
      name: 'Unitary method',
      blurb: 'Find 1% first, then scale up. Clear to follow and easy to check.',
    },
  ],
  ...
  detect(input) {
    ...
    if (
      /interest|invest|loan|compound|deprec|per annum|\bp\.?a\.?\b|years?/i.test(
        input,
      )
    )
      return 0;
  ...
  solve(input, methodId): SolveResult {
    ...
    try {
      ...
      return {
        ok: false,
        error:
          e instanceof Error ? e.message : 'Could not read that percentage.',
      };
    }
    const { kind, a, b } = p;
    ...
    if (kind === 'of') {
      ...
      if (methodId === 'unitary') {
        steps.push({
          ...
          latex: `1\\% \\text{ of } ${fmt(b)} = \\dfrac{${fmt(b)}}{100} = ${fmt(b / 100, 6)}`,
        });
        steps.push({
    ...
    if (kind === 'increase' || kind === 'decrease') {
      ...
      if (methodId === 'unitary') {
        ...
        steps.push({
          note: `Find ${fmt(pct)}% of the amount.`,
          latex: `${fmt(pct)}\\% \\text{ of } ${fmt(amount)} = ${fmt(part)}`,
        });
        steps.push({
